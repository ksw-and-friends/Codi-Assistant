...
async function codiLikeClothing(userId, inputClothing) {
    try {
        ...
        // recommendations와 urls에서 불필요한 공백 제거
...
async function getCodiResults(userId) {
    try {
        ...
        if (!results.length) {
            throw new Error('해당 사용자의 결과를 찾을 수 없습니다.');
        }
        return results;
    } catch (error) {
...
module.exports = { codiLikeClothing, getCodiResults };
